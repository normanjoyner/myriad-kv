    ...
    listen(fn) {
        ...
        this.legiond.on('myriad.sync.full', (message) => {
            if(self.praetor.is_controlling_leader()){
                logger.log('warn', `Rejecting full myriad sync from ${message.author.id}`);
                return;
            }
        ...
        this.legiond.on('myriad.sync.partial', (message) => {
            if(self.praetor.is_controlling_leader()){
                logger.log('warn', `Rejecting partial myriad sync from ${message.author.id}`);
                return;
            }
            ...
            persistence.update_subscribers(message);
        });
