...
module.exports = {
    ...
    update_subscribers: function(message) {
    ...
    bootstrap: function(fn) {
        ...
        if(this.snapshot_eligible) {
            if(_.isEmpty(this.data) && _.isEmpty(this.ttls)) {
    ...
    exists: function(key, options, callback) {
        ...
        if(options.local || this.praetor.is_controlling_leader()) {
            ...
            return callback(new errors.ENOLEADER());
        }
    },
    ...
    set: function(key, value, options, fn) {
        ...
        if(this.praetor.is_controlling_leader()) {
            this.exists(key, { local: true }, (err, exists) => {
                if (err) {
                    return fn(err);
                }

                if (options.if_not_exists && exists === 'true') {
                    return fn(new errors.EEXISTS());
                }
            ...
            var command = options.if_not_exists ? 'SETNX' : 'SET';
            client.connect(leader.management_port, leader.address[this.scope], () => {
                client.write([command, key, value].join(' '));
                client.write(constants.message.DELIMITER);
            });
    ...
    subscribe: function(on_event) {
        this.subscribers.push(on_event);
    },
        ...
        if(options.coalesce) {
            ...
            if(this.sync_ready && this.praetor.is_controlling_leader()) {
                ...
                setTimeout(() => {
                    ...
                    self.update_subscribers(self.sync_options);
            ...
            setImmediate(() => {
                ...
                self.update_subscribers(sync_options);
