...
module.exports = {
    ...
    bootstrap: function(fn){
        ...
        this.snapshot_interval = setInterval(function(){
            type.snapshot({data: self.data, ttls: self.ttls}, function(err){
                if(err)
        ...
        var type = new Disk(this.options);

        if(this.snapshot_eligible){
            type.bootstrap(function(err, snapshot){
                if(err)
                    return fn(err);
    ...
    sync: function(node, options){
        ...
        if(this.sync_ready && this.praetor.is_controlling_leader()){
            ...
            setTimeout(function(){
                ...
                self.praetor.legiond.send(self.sync_options, node);

                self.praetor.legiond.set_attributes({
                    last_sync: new Date().valueOf()
                });

                logger.log("verbose", ["Sending", self.sync_options.event, "to", node && node.host_name || "cluster"].join(" "));
